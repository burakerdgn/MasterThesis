...
int k = 0;
int a = 0;
int tracker12(int argc, char **argv);
...
Results << "frame" << "," << "object number" << "," << "dx" << "," << "dy" << "," << "object center" << "," << "objects class" << endl ; 
//Ptr<Tracker> tracker = TrackerCSRT::create();
	...
	while (1){// (!plot3d.wasStopped()){
		...
		{   
			...
			for (uint32_t k = 0; k < cstereoVisionForADAS->m_vecobjBB.size() ; ++k)
				{
					if (cstereoVisionForADAS->m_vecobjBB[k].rectBB.height > 80)
					{
						if(cstereoVisionForADAS->m_vecobjBB[k].rectBB.width > 70){
							...
							bbox.x = (cstereoVisionForADAS->m_vecobjBB[k].rectBB.x / 2)-5;
			...
			//cout << "bb size :" << cstereoVisionForADAS->m_vecobjBB.size() << endl;
			//cout << "filtered" << bboxes.size() << endl ;
			getRandomColors(colors, bboxes.size());
			for(int i=0; i < bboxes.size(); i++){
    		//multiTracker->add(createTrackerByName(trackerType), frame, Rect2d(bboxes[i]));
   				multiTracker->add(TrackerMOSSE::create(), imgLeft, Rect2d(bboxes[i])); 
			...
			//bbox = selectROI("tracker", imgLeft);
            
			//rectangle(imgLeft, pt, pt1 ,Scalar( 255, 0, 0 ), 2, 1 );
			imshow("MultiTracker", imgLeft);
			//rectangle(imgLeft, bbox ,Scalar( 255, 0, 0 ), 2, 1 );
			//tracker->init(imgLeft, bbox);
